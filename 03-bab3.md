...
4. Sistem harus memvalidasi setiap berkas yang diunggah oleh pengguna (seperti formulir APL-01, APL-02, tugas asesmen, dan sertifikat digital) dengan menerapkan pembatasan tipe berkas (file extension berupa PDF, JPG, atau PNG) dan batas ukuran maksimal berkas (misalnya 2 MB atau 5 MB) untuk mencegah injeksi malware maupun server overload.

5. Sistem harus memproteksi direktori penyimpanan berkas agar tidak terekspos ke publik (directory traversal protection), sehingga berkas pendaftaran dan sertifikat hanya dapat diakses atau diunduh oleh pengguna yang memiliki hak akses (autentikasi dan otorisasi) yang sah.
...
3. Pengiriman *Push Notification* harus diproses dan didistribusikan secara real-time dengan waktu jeda (latency) yang seminimal mungkin.

C. Skalabilitas

1. Sistem harus mampu menangani peningkatan jumlah pengguna (concurrent users) yang mengakses aplikasi secara bersamaan, terutama pada masa puncak pendaftaran skema sertifikasi.
...
2. Antarmuka sistem harus bersifat responsif, sehingga tampilan dapat menyesuaikan dengan mulus pada berbagai ukuran layar perangkat, mulai dari desktop (komputer/laptop) hingga *mobile*.
...
Bagi Asesi dengan status pendaftaran yang telah divalidasi, Asesor akan membuat dan memberikan tugas asesmen melalui sistem. Asesi kemudian wajib mengerjakan instruksi tersebut dan mengumpulkan jawaban atau hasil tugas asesmen kembali ke dalam sistem untuk dinilai.

3. Tahap Penilaian dan Penerbitan Sertifikat

Setelah Asesi mengumpulkan tugas, Asesor akan mengevaluasi hasil tersebut dan menetapkan status final kelulusan. Apabila Asesi dinilai "Belum Kompeten", maka proses pendaftaran selesai sampai di tahap ini. Sebaliknya, apabila Asesi dinilai "Kompeten", Admin akan mengunggah berkas sertifikat kelulusan ke dalam sistem. Pada tahap akhir, Asesi dapat mengunduh sertifikat tersebut secara langsung, dan proses bisnis dinyatakan selesai.
...
#### Spesifikasi *Use Case* Lihat Detail Asesi
...
Gambar \ref{fig:activity-kelola-asesi} menunjukkan proses transisi status Asesi, seperti berkas sudah lengkap, diberikan hak akses ke menu asesmen, atau dinyatakan kompeten berdasarkan hasil verifikasi dokumen atau hasil keputusan asesmen oleh Admin/Asesor.
...
\begin{longtable}{|p{2.5cm}|p{2.6cm}|p{2cm}|>{\raggedright\arraybackslash}p{5.5cm}|}
\caption{Tabel Users} \\
\hline
...
\begin{longtable}{|p{3cm}|p{2.6cm}|p{2cm}|>{\raggedright\arraybackslash}p{5.5cm}|}
...
\hline
Jenis Perangkat & Merek / Tipe & Peran Pengujian \\ \hline
Laptop / PC & Acer Aspire A314-32 & Perangkat utama (Aktor Admin/Asesor), \textit{server} lokal, dan pemicu notifikasi \\ \hline
...
\hline
Perangkat Lunak & Spesifikasi / Versi & Keterangan \\ \hline
